...
interface Logo3DAntimatterProps {
  scale?: number;
  mousePosition?: React.MutableRefObject<{ x: number; y: number }>;
}

const Logo3DAntimatter = ({ scale = 1, mousePosition }: Logo3DAntimatterProps) => {
  ...
  // Antimatter Indigo/Purple color palette
  const primaryColor = '#818cf8'; // Indigo 400
  const accentColor = '#a5b4fc';  // Indigo 300
  const glowColor = '#6366f1';    // Indigo 500
  const successColor = '#a78bfa'; // Purple 400
  ...
  return (
    <Float speed={1.2} rotationIntensity={0.03} floatIntensity={0.15}>
      <group ref={groupRef} scale={scale}>
        ...
        {/* Chart line - main trading symbol - Indigo */}
        ...
        {/* Arrow head at chart top - Purple */}
        ...
        {/* Large checkmark - CLEARED - Purple accent */}
        ...
        {/* Corner accent dots - Indigo */}
        <mesh position={[0.85, 0.65, 0.1]}>
          <sphereGeometry args={[0.05, 16, 16]} />
          <meshStandardMaterial color={accentColor} emissive={glowColor} emissiveIntensity={2.5} />
        </mesh>
        <mesh position={[-0.85, 0.65, 0.1]}>
          <sphereGeometry args={[0.05, 16, 16]} />
          <meshStandardMaterial color={accentColor} emissive={glowColor} emissiveIntensity={2.5} />
...
export default Logo3DAntimatter;
