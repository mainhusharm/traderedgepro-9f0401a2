 ...
 export const AIKeySettings = ({ onKeyConfigured, compact = false }: AIKeySettingsProps) => {
   ...
   const handleSave = async () => {
     ...
     try {
       ...
       const encryptedKey = btoa(`${provider}:${apiKey}`);
     ...
     return (
       <div className="flex items-center gap-2 text-sm">
         ...
         <span className="text-muted-foreground">
           Nexus powered by {AI_PROVIDERS.find(p => p.id === existingSettings.provider)?.name || existingSettings.provider}
         </span>
      ...
      <Card className={compact ? 'border-dashed border-border bg-muted/30' : ''}>
       <CardHeader className={compact ? 'pb-3' : ''}>
         <CardTitle className={`flex items-center gap-2 ${compact ? 'text-base' : ''}`}>
           ...
           {existingSettings?.hasKey ? 'Update AI Engine' : 'Connect AI Engine'}
         </CardTitle>
         <CardDescription>
           {existingSettings?.hasKey
             ? 'Change the AI engine powering your Nexus coach'
             : 'Connect an AI provider to power Nexus. We combine it with our proprietary trading intelligence.'}
         </CardDescription>
       </CardHeader>
       <CardContent className="space-y-4">
         ...
         <div className="space-y-2">
           ...
           <p className="text-xs text-muted-foreground">
             Your key powers Nexus's AI reasoning. We enhance it with our SMC analysis, market data, and trading logic.
           </p>
         </div>
         ...
         {existingSettings?.hasKey && (
            <div className="flex items-center gap-2 p-3 rounded-lg bg-primary/10 border border-primary/20">
             ...
             <div>
               ...
               <p className="text-xs text-muted-foreground">
                 Powered by {AI_PROVIDERS.find(p => p.id === existingSettings.provider)?.name} + TraderEdge Intelligence
               </p>
             </div>
           </div>
         )}
