...
const AgentManagement = ({ onOnlineAgentsChange }: AgentManagementProps) => {
  ...
  const handleInviteAgent = async () => {
    ...
    try {
      ...
      // Send invitation email via callEdgeFunction (routes through Render backend in production)
      const { error: emailError } = await callEdgeFunction('send-agent-invitation', {
        email: formData.email,
        name: formData.name,
        invitationToken,
        permissions: {
      ...
      if (emailError) {
        ...
        toast.error('Agent created but invitation email failed to send. You can copy the invite link instead.');
