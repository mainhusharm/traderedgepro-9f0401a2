...
const SESSION_KEY = 'marketing_session';

export const MarketingRoute = ({ children }: MarketingRouteProps) => {
  ...
  useEffect(() => {
    ...
    if (session) {
      ...
      // Token is generated server-side; we only validate local expiry client-side.
    ...
    // Check for lockout
    const lockout = localStorage.getItem('marketing_lockout');
    if (lockout) {
      ...
      if (Date.now() < lockoutEnd) {
        ...
        localStorage.removeItem('marketing_lockout');
      }
    }
  }, []);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const { data, error: fnError } = await supabase.functions.invoke('validate-marketing-access', {
        body: { mpin: pin }
      });
      ...
      if (data?.success) {
        ...
        if (newAttempts >= 5) {
          ...
          localStorage.setItem('marketing_lockout', lockoutEnd.toString());
          setIsLocked(true);
          setLockoutEndTime(lockoutEnd);
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              <div>
                <input
                  ...
                  disabled={isLocked}
                  className="w-full px-4 py-4 text-center text-2xl tracking-[0.5em] font-mono bg-background border border-border rounded-xl focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent disabled:opacity-50 disabled:cursor-not-allowed"
                />
              </div>
...
export const useMarketingLogout = () => {
  const navigate = useNavigate();
