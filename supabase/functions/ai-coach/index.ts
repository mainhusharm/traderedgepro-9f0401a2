 ...
 async function getUserAISettings(supabase: any, userId: string): Promise<{ provider: string; apiKey: string } | null> {
   try {
     ...
     try {
       ...
       const apiKey = apiKeyParts.join(':'); // Handle keys with colons
       if (provider && apiKey) {
...
serve(async (req) => {
  ...
  try {
    ...
    const { messages, userContext, userId, includeTradeContext = true, signalContext } = await req.json();
    ...
    if (!Array.isArray(messages) || messages.length === 0) {
     ...
     if (supabase && userId) {
       userAISettings = await getUserAISettings(supabase, userId);
       if (userAISettings) {
    ...
    let tradeContext = '';
    if (includeTradeContext && supabase && userId) {
      const tradeData = await fetchUserTradeContext(supabase, userId);
