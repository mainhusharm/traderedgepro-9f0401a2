...
serve(async (req) => {
  ...
  try {
    ...
    const { userId, milestoneName, milestoneValue, milestoneType } = await req.json();

    if (!userId || !milestoneName) {
      return new Response(JSON.stringify({ error: "Missing userId or milestoneName" }), {
        status: 400,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
    ...
    const logData = {
      ...
      body: milestoneValue ? `You've reached ${milestoneValue}!` : "Congratulations on your achievement!",
      status: "pending",
    ...
    const payload = {
      ...
      data: { url: "/dashboard?tab=performance", type: "milestone", milestoneName, milestoneType },
    };
    ...
    const { endpoint, headers, body: encryptedBody } = await buildPushHTTPRequest({
      ...
      message: {
        payload,
        options: { ttl: 86400, urgency: "high", topic: "milestone" },
